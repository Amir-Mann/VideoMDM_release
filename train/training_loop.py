...
from torch.nn.utils import clip_grad_norm_ as torch_clip_grad_norm
...
from eval import eval_humanml, eval_humanact12_uestc, eval_video_mdm
...
class TrainLoop:
    def __init__(self, args, train_platform, model, diffusion, data, other_data=None, model_2d=None):
        self.args = args
        self.dataset = args.dataset
        self.train_platform = train_platform
        self.model = model
        self.model_avg = None
        if not self.args.dont_use_ema:
            self.model_avg = copy.deepcopy(self.model)
        self.model_for_eval = self.model_avg if not self.args.dont_use_ema else self.model
        if args.gen_guidance_param != 1:
            #print(self.model, self.model_avg, self.model_for_eval)
        ...
        if self.resume_step and not self.args.fine_tunning:
        ...
        self.default_schedule_sampler = create_named_schedule_sampler(self.schedule_sampler_type, diffusion)
        self.eval_wrapper, self.eval_data, self.eval_gt_data = None, None, None
        if args.dataset in ['kit', 'humanml', 'video_mdm_synthetic', 'video_mdm_synthetic_mvlift'] and args.eval_during_training:
            return_to_video_mdm_synthetic = False
            if args.dataset == 'video_mdm_synthetic':
                args.dataset = 'humanml'
                return_to_video_mdm_synthetic = True
            elif args.dataset == "video_mdm_synthetic_mvlift":
                args.dataset = 'humanml'
                return_to_video_mdm_synthetic = "mvlift"
            ...
            self.eval_data = {
                'model': lambda: eval_humanml.get_mdm_loader(
                    ...
                    max_motion_length=gen_loader.dataset.opt.max_motion_length,
                    num_samples_limit=args.eval_num_samples,
                    scale=args.gen_guidance_param
                )
            }
            self.evaluation_func = eval_humanml.evaluation
            if return_to_video_mdm_synthetic:
                args.dataset = 'video_mdm_synthetic'
                if return_to_video_mdm_synthetic == "mvlift":
                    args.dataset = 'video_mdm_synthetic_mvlift'
        elif 'egoexo' in args.dataset and args.eval_during_training:
            mm_num_samples = 0  # mm is super slow hence we won't run it during training
            mm_num_repeats = 0  # mm is super slow hence we won't run it during training
            """
            ...
            """
            self.eval_gt_data = get_dataset_loader(name='egoexo', batch_size=args.eval_batch_size, num_frames=None, split=args.eval_split, hml_mode='2d')
            gen_loader = get_dataset_loader(name='egoexo', batch_size=args.eval_batch_size, num_frames=None, split=args.eval_split, hml_mode='2d')
            self.eval_data = {
                'model': lambda: eval_video_mdm.get_mdm_loader(
                    model=self.model_for_eval,
                    diffusion=self.diffusion,
                    batch_size=args.eval_batch_size,
                    ground_truth_loader=gen_loader,
                    ...
                    num_samples_limit=args.eval_num_samples,
                    scale=args.gen_guidance_param,
                    collate_fn_to_use=eval_video_mdm.tensors_t2m_collate
                )
            }
            self.eval_wrapper = eval_video_mdm.Evaluator('egoexo', dist_util.dev(), args.evaluator_dir_path)
            self.evaluation_func = functools.partial(
                eval_video_mdm.evaluation,
                num_samples_limit=args.eval_num_samples,
            )
    ...
    def run_loop(self):
        ...
        for epoch in tqdm(range(self.num_epochs)):
            ...
            for motion, cond in generator:
                ...
                if (self.other_data is not None) and self.args.video_mdm:
                    self.optimize_other_now = generator.was_last_other_data()
                    self.diffusion.loss_type = LossType.MSE if self.optimize_other_now else LossType.CAMERA_MSE
                #print(f'step {self.step} loss type {self.diffusion.loss_type} optimize_other_now {self.optimize_other_now} args.dataset {self.dataset}')
                if self.args.quicker_batch_chance > 0 and np.random.rand() < self.args.quicker_batch_chance:
                    original_length = motion.shape[-1]
                    motion = motion[..., :100]
                    cond['y']['mask'] = cond['y']['mask'][..., :100]
                    # work on second dim for tensors of shape [bs, nframes, njoints, ?]
                    assert cond['extra']['motion_2d'].shape[1] == original_length, f"bug in dimentions, {original_length} != {cond['extra']['motion_2d'].shape[1]}"
                    assert cond['extra']['scores_2d'].shape[1] == original_length, f"bug in dimentions, {original_length} != {cond['extra']['scores_2d'].shape[1]}"
                    cond['extra']['motion_2d'] = cond['extra']['motion_2d'][:, :100]
                    cond['extra']['scores_2d'] = cond['extra']['scores_2d'][:, :100]
            ...
            if not (not self.lr_anneal_steps or self.total_step() < self.lr_anneal_steps):
                break
            if self.args.sanity_check_folder is not None:
                self.handle_sanity_check()
    ...
    def evaluate(self):
        ...
        if self.eval_wrapper is not None:
            ...
            eval_dict = self.evaluation_func(
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            if self.other_data and self.optimize_other_now:
                t, weights = self.default_schedule_sampler.sample(micro.shape[0], dist_util.dev())
            else:
            ...
            compute_losses = functools.partial(
                ...
                distilation_model=self.model_2d
            )
            ...
            log_loss_dict(
                self.diffusion, t, {k: v * weights for k, v in losses.items()}, log_quartiles=(0 if self.args.video_mdm else 4)
            )
            if self.other_data and self.optimize_other_now:
                self.other_mp_trainer.backward(loss)
                torch_clip_grad_norm(self.other_mp_trainer.master_params, max_norm=self.args.clip_grad_max_norm)
            else:
                self.mp_trainer.backward(loss)
                torch_clip_grad_norm(self.mp_trainer.master_params, max_norm=self.args.clip_grad_max_norm)
    ...
    def save(self):
        def save_checkpoint():
            ...
            save_path = os.path.join(self.save_dir, self.ckpt_file_name())
        ...
        with open(
            os.path.join(self.save_dir, f"opt{(self.total_step()):09d}.pt"),
            "wb",
        ) as f:
    ...
    def handle_sanity_check(self):
        """
        This function is called on the first epoch of sanity check mode runs, and exits at it's end.
        If the sanity_check_folder doesn't exists, it creates it and documents the model state in it.
        If it exists, it compares the current model state to the one in the folder.
        """
        if os.path.exists(self.args.sanity_check_folder):
            # compare current model to the one in the folder
            checkpoint_path = os.path.join(self.args.sanity_check_folder, "model_sanity_check.pt")
            state_dict = dist_util.load_state_dict(
                checkpoint_path, map_location=dist_util.dev())
            current_state_dict = self.model.state_dict()
            for key in state_dict:
                if not torch.equal(state_dict[key], current_state_dict[key]):
                    print(f"Sanity check failed: parameter {key} differs from the saved checkpoint.")
            else:
                print("Sanity check passed: all parameters match the saved checkpoint.")
        else:
            # create the folder and save the model state
            os.makedirs(self.args.sanity_check_folder, exist_ok=True)
            checkpoint_path = os.path.join(self.args.sanity_check_folder, "model_sanity_check.pt")
            state_dict = self.model.state_dict()
            with open(checkpoint_path, "wb") as f:
                torch.save(state_dict, f)
            print(f"Sanity check folder created at {self.args.sanity_check_folder}. Model state saved.")

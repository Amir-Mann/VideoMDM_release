...
def extract_features(positions, feet_thre, n_raw_offsets, kinematic_chain, face_joint_indx, fid_r, fid_l,
                     smooth_forward=True, return_everything=False):
    ...
    def get_quaternion(positions):
        ...
        quat_params = skel.inverse_kinematics_np(positions, face_joint_indx, smooth_forward=smooth_forward)
...
def process_file(positions, feet_thre, alternative_tgt_offsets=None):
    # (seq_len, joints_num, 3)
    #     '''Down Sample'''
    #     positions = positions[::ds_num]

    '''Uniform Skeleton'''
    if alternative_tgt_offsets is None:
        positions = uniform_skeleton(positions, tgt_offsets)
    else:
        positions = uniform_skeleton(positions, alternative_tgt_offsets)
...
def get_xyz_hunanml(sample, mean, std):
    """
    Recover XYZ *positions* from HumanML3D vector representation
    sample [bs, vec_dim, 1, nframes]
    mean [vec_dim]
    std [vec_dim]
    return [bs, njoints, 3, nframes]
    """
    n_joints = (sample.shape[1] + 1) // 12
    sample = sample.permute(0, 2, 3, 1) * std + mean

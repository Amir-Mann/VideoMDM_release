...
def plot_3d_motion(save_path, kinematic_tree, joints, title, dataset, figsize=(3, 3), fps=120, radius=3,
                   vis_mode='default', gt_frames=[], other_joints=None, static_frame=False):
    #joints shape should be [nframes, njoints, 3]
    matplotlib.use('Agg')
    ...
    if static_frame:
        MINS = MINS.min() * np.ones_like(MINS)
        MAXS = MAXS.max() * np.ones_like(MAXS)
    ...
    if save_path is not None:
        # set and unset ani duration
        ani.duration = data.shape[0] / fps
        ani.write_videofile(save_path, fps=fps)
        ani.duration = None

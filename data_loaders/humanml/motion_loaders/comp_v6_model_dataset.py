...
class CompMDMGeneratedDataset(Dataset):
    ...
    def __init__(self, model, diffusion, dataloader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale=1.):
        ...
        if hasattr(self.dataset, 't2m_dataset'):
            self.t2m_dataset = self.dataset.t2m_dataset
        ...
        with torch.no_grad():
            for i, (motion, model_kwargs) in tqdm(enumerate(dataloader)):
                ...
                if num_samples_limit and motions_count + motion.shape[0] > num_samples_limit:
                    break
                motions_count += motion.shape[0]
                ...
                for t in range(repeat_times):
                    ...
                    if hasattr(diffusion.args, 'reconstruct_hmlvec_from_ric') and diffusion.args.reconstruct_hmlvec_from_ric:
                        tt = lambda x: torch.tensor(x).to(sample.device)
                        sample_xyz = get_xyz_hunanml(sample, tt(self.dataset.t2m_dataset.mean), tt(self.dataset.t2m_dataset.std))
                        sample_xyz = sample_xyz.permute(0, 3, 1, 2)
                        sample = process_file_torch(sample_xyz)[0].unsqueeze(2).permute(0, 3, 2, 1)
                        
                        tt_unsq = lambda x: tt(x).unsqueeze(0).unsqueeze(2).unsqueeze(3)
                        mean, std = tt_unsq(self.dataset.t2m_dataset.mean), tt_unsq(self.dataset.t2m_dataset.std)
                    ...
                    if t == 0:
                        sub_dicts = [{
                            ...
                            } for bs_i in range(motion.shape[0])]
                ...
                if is_mm:
                    mm_generated_motions += [{
                                    ...
                                    'mm_motions': mm_motions[bs_i::motion.shape[0]],  # collect all 10 repeats from the (32*10) generated motions
                                    } for bs_i in range(motion.shape[0])]

...
def get_mdm_loader(model, diffusion, batch_size, ground_truth_loader, mm_num_samples, mm_num_repeats, 
                   max_motion_length, num_samples_limit, scale, collate_fn_to_use=collate_fn):
    opt = {
        'name': 'test',  # FIXME
    }
    print('Generating %s ...' % opt['name'])
    ...
    # NOTE: bs must not be changed! this will cause a bug in R precision calc! (Legacy from MDM, Precision and Recall now use different code)
    motion_loader = DataLoader(dataset, batch_size=batch_size, collate_fn=collate_fn_to_use, drop_last=ground_truth_loader.drop_last, num_workers=4)
    mm_motion_loader = DataLoader(mm_dataset, batch_size=1, num_workers=1)

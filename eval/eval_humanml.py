...
import numpy as np
import torch
from data_loaders.humanml.motion_loaders.model_motion_loaders import get_mdm_loader, collate_fn  # get_motion_loader
...
from eval.VideoMDM.mas.eval_mas_dataset import get_mas_loader, humanml_eval_collate_fn
...
def evaluate_matching_score(eval_wrapper, motion_loaders, file):
    ...
    for motion_loader_name, motion_loader in motion_loaders.items():
        ...
        with torch.no_grad():
            print("motion_loader_name:", motion_loader_name, "length:", len(motion_loader))
            progress = tqdm(list(range(len(motion_loader))), desc=f"Evaluating {motion_loader_name}")
...
if __name__ == '__main__':
    ...
    logger.log(f"creating data loader for {dataset}...")
    split = 'test'
    
    gt_loader = get_dataset_loader(name=dataset, batch_size=args.batch_size, num_frames=None, split=split, hml_mode='gt')
    ...
    gen_loader = get_dataset_loader(name=dataset, batch_size=args.batch_size, num_frames=None, split=split, hml_mode='eval', device=dist_util.dev())
    ...
    if args.mas_data_dir is not None:
        if args.skip_generation:
            eval_motion_loaders = {}
        norm_params = (gen_loader.dataset.mean, gen_loader.dataset.std)
        eval_motion_loaders['mas'] = lambda: (get_mas_loader(args, collate_fn=humanml_eval_collate_fn, w_vectorizer=gen_loader.dataset.w_vectorizer, norm_params=norm_params), None)

    eval_wrapper = EvaluatorMDMWrapper(dataset, dist_util.dev())
    evaluation(eval_wrapper, gt_loader, eval_motion_loaders, log_file, replication_times, diversity_times, mm_num_times, run_mm=run_mm, eval_platform=eval_platform)
    eval_platform.close()

...
def load_model_wo_clip(model, state_dict):
    ...
    assert len(unexpected_keys) == 0, f"There shoudl be none, but got the following unexpected keys: {unexpected_keys}"
...
def get_model_args(args, data):
    ...
    if hasattr(data, 'dataset') and hasattr(data.dataset, 'num_actions'):
    ...
    if 'humanml' in args.dataset or 'egoexo' in args.dataset or 'video_mdm_synthetic' in args.dataset or 'fit3d' in args.dataset:
        data_rep = 'hml_vec'
        njoints = 22 * 12 - 1
        nfeats = 1

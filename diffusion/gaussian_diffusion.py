...
from diffusion.video_mdm_diffusion import calculate_video_mdm_loss, project_points_to_ray_hmlvec, mask_refactor_to_hmlvec
from data_loaders.humanml.scripts import motion_process
from utils.math_utils import axis_rotation_matrix
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        **kargs
        ...
        self.args.lambda_root_vel = 0.
        self.args.lambda_vel_rcxyz = 0.

        if self.args.lambda_rcxyz > 0. or self.args.lambda_vel > 0. or self.args.lambda_root_vel > 0. or \
                self.args.lambda_vel_rcxyz > 0. or self.args.lambda_fc > 0.:
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        assert noise.shape == x_start.shape, f"noise.shape {noise.shape} but x_start.shape {x_start.shape}"
        return (
            _extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape) * x_start
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None, distilation_model=None):
        """
        ...
        :param dataset: A dataset class to get parameters for humanml format reverting
        :param distilation_model: An option 2d diffusion model to use for SDS loss to supervise trainig.
        ...
        if len(t.shape) != 1: # For multistep t* training
            first_t = t[:, 0]
            assert first_t.shape == x_start.shape[:1], f"first_t.shape became {first_t.shape} but x_start.shape is {x_start.shape}"
        else:
            first_t = t

        if self.args.video_mdm and self.args.gt_teacher:
            x_t = self.q_sample(extra['gt_motion'].to(x_start.device), first_t, noise=noise)
        else:
            x_t = self.q_sample(x_start, first_t, noise=noise)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.args.lambda_vel_rcxyz > 0.:
            ...
            terms["loss"] = terms["rot_mse"] + terms.get('vb', 0.) +\
                            (self.args.lambda_vel * terms.get('vel_mse', 0.)) +\
                            (self.args.lambda_rcxyz * terms.get('rcxyz_mse', 0.)) + \
                            (self.args.lambda_fc * terms.get('fc', 0.))
        elif self.loss_type == LossType.CAMERA_MSE: # AMIR MARKER
            scores = extra['scores_2d'].to(mask.device)
            pad_len = mask.shape[-1] - scores.shape[1]
            scores_padded = torch.nn.functional.pad(scores, (0, 0, 0, pad_len), value=1.0)
            scores_padded = scores_padded.permute(0, 2, 1).unsqueeze(2) # [bs, njoints, 1, nframes]
            if self.args.mask_from_score_type == 'square':
                mask = mask * (scores_padded ** 2) # [bs, njoints, 1, nframes]
            elif self.args.mask_from_score_type == 'threshold4':
                mask = mask * (scores_padded > 0.4).to(mask.dtype) # [bs, njoints, 1, nframes]
            elif self.args.mask_from_score_type == 'threshold6':
                mask = mask * (scores_padded > 0.6).to(mask.dtype) # [bs, njoints, 1, nframes]
            elif self.args.mask_from_score_type == 'linear':
                mask = mask * (scores_padded) # [bs, njoints, 1, nframes]
            ...
            if self.dataset_params_not_initialized:
                ...
                self.dataset_torch_std = torch.tensor(dataset.t2m_dataset.std).to(x_t.device).float()
                self.dataset_torch_mean = torch.tensor(dataset.t2m_dataset.mean).to(x_t.device).float()
                self.hml_args = (self.dataset_torch_mean, self.dataset_torch_std)
                self.sqrt_alphas_torch = torch.from_numpy(self.sqrt_alphas_cumprod).to(x_t.device).float()
                self.sqrt_one_minus_alphas_torch = torch.from_numpy(self.sqrt_one_minus_alphas_cumprod).to(x_t.device).float()
            def extract_timestemp(t, broadcast_shape):
                shape = (-1,) + (1,) * (len(broadcast_shape) - 1)
                sqrt_alphas = self.sqrt_alphas_torch[t].reshape(shape)
                sqrt_one_minus_alphas = self.sqrt_one_minus_alphas_torch[t].reshape(shape)
            ...
            if len(t.shape) == 1:
                model_output = model(x_t, self._scale_timesteps(t), **model_kwargs)
                loss += calculate_video_mdm_loss(x_t, t, model_output, model_kwargs, mask, extra, terms, self.args, self.debug, distilation_model, hml_args=self.hml_args)
            else:
                assert t.shape[0] == x_start.shape[0], "t and x_start must have the same batch size"
                for i in range(t.shape[1]): # For multistep t* training
                    model_output = model(x_t, self._scale_timesteps(t[:, i]), **model_kwargs)
                    if i == t.shape[1] - 1 or "e1" in self.args.t_star_method:
                        loss_i = calculate_video_mdm_loss(x_t, t[:, i], model_output, model_kwargs, mask, extra, terms, self.args, self.debug, distilation_model, f"{i}_", hml_args=self.hml_args)
                        loss += (1 / t.shape[1] if "e1" in self.args.t_star_method else 1) * loss_i
                    ...
                    t1 = t[:, i]
                    t2 = t[:, i + 1]
                    sqrt_alpha_t1, sqrt_one_minus_alpha_t1 = extract_timestemp(t1, x_t.shape)
                    sqrt_alpha_t2, sqrt_one_minus_alpha_t2 = extract_timestemp(t2, x_t.shape)
                    if self.args.multistep_use_guidance:
                        x_0 = project_points_to_ray_hmlvec(
                            hmlvec=model_output, diffusion_extra=extra, hml_args=self.hml_args,
                            #orthographic=self.args.dataset in ["nba"]
                            dataset=self.args.dataset if self.args.dataset == "nba" else None
                        )
                    else:
                        x_0 = model_output
                    epsilon_t1 = (x_t - x_0 * sqrt_alpha_t1) / sqrt_one_minus_alpha_t1
                    x_t = sqrt_alpha_t2 * x_0 + sqrt_one_minus_alpha_t2 * epsilon_t1

                    if "d1" in self.args.t_star_method:
                        x_t = x_t.detach()

            terms["loss"] = loss
            if self.debug:
                terms["debug"]["loss"] = loss
                terms["debug"]["t"] = t
                terms["debug"]["text"] = model_kwargs["y"]["text"]
        else:
...
def _extract_into_tensor(arr, timesteps, broadcast_shape):
    ...
    """
    if timesteps.max() >= arr.shape[0]:
        print(f'timesteps: {timesteps.max()} >= arr.shape[0]: {arr.shape[0]}')
        print(f'arr: {arr}')
